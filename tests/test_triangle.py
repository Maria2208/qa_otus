...
def test_square_has_negative_base():
    ...
    with pytest.raises(ValueError):
        Triangle(name="треугольник", base=-1, height=2, first_side=1, second_side=2).check_values()


def test_square_has_string_base():
    """Метод проверяет, что выбрасывается исключение, если в base передать строковое значение"""
    with pytest.raises(ValueError):
        Triangle(name="треугольник", base="abc", height=2, first_side=1, second_side=2).check_values()
...
def test_square_has_string_height():
    ...
    with pytest.raises(ValueError):
        Triangle(name="треугольник", base=1, height="abc", first_side=1, second_side=2).check_values()
...
def test_square_has_string_first_side():
    ...
    with pytest.raises(ValueError):
        Triangle(name="треугольник", base=1, height=1, first_side="abc", second_side=2).check_values()
...
def test_square_has_string_second_side():
    ...
    with pytest.raises(ValueError):
        Triangle(name="треугольник", base=1, height=1, first_side=2, second_side="abc").check_values()
